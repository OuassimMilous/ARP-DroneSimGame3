...
void readSocket(int client_socket) {
 do
 {
   // Read the message from the client
    ...
    sprintf(msg,"[Server]:Sent echo back to server: %s", bufferrec);
    logit(msg);
...
void handle_socket_reced(int client){
    ...
    if (strcmp(reced, "TI") == 0) {
        ...
        char currentNumber[NUM_TARGETS*2];  // Assuming a fixed-size array for simplicity
        int numbersCount = 0;
        ...
        // Code for first letter 'T'
        // parse_recieved(reced);
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    while(1){
        ...
        // if we won, restart
        if (data.targetReached == -1)
        {
            /* code */
            writeSocket("GE",client_socket);
        }
